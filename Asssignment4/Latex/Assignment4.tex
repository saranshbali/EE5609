...
\end{align}
\section{Explaination about Pair of Straight Lines}
The general equation of second degree is given by
\begin{align}
&	ax^2 + 2bxy + cy^2 + 2dx + 2ey + f = 0 \label{1}
\intertext{and the \eqref{1} can be expressed as} 
&\vec{x}^T\vec{V}\vec{x}+2\vec{u}^T\vec{x}+f=0 \label{2}
\intertext{where}
& \qquad \vec{V} = \vec{V}^T = \myvec{a & b \\ b & c} \\
& \qquad \vec{u} = \myvec{d \\ e}
\end{align}

\eqref{1} refers a pair of staright lines if
\begin{align}
	\begin{array}{|cc|}
		\vec{V} & \vec{u}\\\vec{u}^T & f
	\end{array}&=0\label{3}
\end{align}
Otherwise \eqref{1} represents a conic.
\section{Solution}
Here we are given 
\begin{align}
	2x^2+ xy -y^2 + kx + 6y - 9 = 0 \label{4} 
\end{align}
We need to find the value of k for which \eqref{4} represents a pair of straight lines.

From \eqref{1} and \eqref{2}, we can convert \eqref{3} as 
\begin{align}
	\vec{x}^T\myvec{2 & 1/2 \\ 1/2 & -1}\vec{x}+2\myvec{k/2 \\ 3}\vec{x}-9=0 
\end{align} 
Here, we have
\begin{align}
...
& f = -9
\end{align}
Since \eqref{4} represents a pair of straight lines, then by \eqref{3}, we have
\begin{align}
	\begin{array}{|ccc|}
